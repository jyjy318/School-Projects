...
int check_cache_data_hit(void *addr, char type) {
	int cache_index = (((int)addr / 8) % CACHE_SET_SIZE);
	int tag = (((int)addr / 8) / CACHE_SET_SIZE);
...
int find_entry_index_in_set(int cache_index) {
	...
	int temp = cache_array[cache_index][0].timestamp;
	for (int i = 0; i < DEFAULT_CACHE_ASSOC; i++) {
		if (cache_array[cache_index][i].timestamp <= temp) {
			temp = cache_array[cache_index][i].timestamp;
			entry_index = i;
		}
	}
    return entry_index; 
...
int access_memory(void *addr, char type) {
	...
	if (type == 'b')
		...
		(int *)x;
	memmove(&cache_array[cache_index][entry_index].data[0], &memory_array[mem_index], sizeof(x));
	memmove(&cache_array[cache_index][entry_index].data[4], &memory_array[mem_index + 1], sizeof(x));
	cache_array[cache_index][entry_index].valid = 1;
	cache_array[cache_index][entry_index].tag = ((int)addr / 8) / CACHE_SET_SIZE;
	cache_array[cache_index][entry_index].timestamp = time;
	time += 1;
    return 0;
}
